/* MCP bridge: lets a local MCP server (mcp/server.mjs) drive the app so AI
   assistants can build scenes. The page connects OUT to a WebSocket the
   server opens on 127.0.0.1, receives {id, cmd, args} frames, runs them
   against the BT API and answers {id, ok, result|error}. Nothing connects
   unless the user turns on the plug button in the top bar; the choice is
...
BT.MCP = {
  ...
  init() {
    try { this.enabled = localStorage.getItem(this.KEY) === "1"; } catch (_) {}
  ...
  toggle() {
    ...
    if (this.enabled) { this._retry = 0; this._connect(); }
  ...
  _connect() {
    if (this._ws) return;
    let ws;
    try { ws = new WebSocket("ws://127.0.0.1:" + this.PORT); } catch (_) { this._scheduleRetry(); return; }
    this._ws = ws;
    ws.onopen = () => {
      this._retry = 0;
      this.connected = true;
      ...
      BT.emit("mcp");
      BT.emit("toast", "MCP connected: an AI assistant can now edit this scene");
    };
  ...
  _scheduleRetry() {
    ...
    this._timer = setTimeout(() => this._connect(), Math.min(1000 * Math.pow(2, this._retry++), 15000));
  },
